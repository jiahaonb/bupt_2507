...
class DynamicVFE(nn.Module):
    ...
    def __init__(self,
        ...
        self.vfe_scatter = DynamicScatter(voxel_size, point_cloud_range,
                                          (mode != 'max'))
        self.vfe_scatter_img = DynamicScatter(voxel_size, point_cloud_range,
                                              (mode != 'max'))
        self.cluster_scatter = DynamicScatter(
            voxel_size, point_cloud_range, average_points=True)

        # self.fusion_layer = DynamicVFELayer(
        #     feat_channels[-1]*2,
        #     feat_channels[-1],
        #     norm_cfg)
        if fusion_layer is not None:
        ...
        self.pointaugmenting = pointaugmenting
        # if pointaugmenting:
        ...
        #                 out_filters,
        #                 norm_cfg))
        #     self.vfe_layers_img = nn.ModuleList(vfe_layers)
    ...
    def forward(self,
        ...
        if mode=='MVXNet':
            ...
            # for i, vfe in enumerate(self.vfe_layers_img):
            #     point_feats = vfe(features)
            voxel_feats, voxel_coors = self.vfe_scatter_img(features, coors)
